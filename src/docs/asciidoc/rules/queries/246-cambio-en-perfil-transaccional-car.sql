...
WHERE YEAR = ?
  ...
  AND (
        (((cast(NO_OPERACIONES_DEPOSITO AS DECIMAL) * 1.#percent) - ACT_NO_TOT_DEPOSITOS) < 0 AND
         NO_OPERACIONES_DEPOSITO > 0) OR
        (((cast(IMP_MONTO_ESTIMADO_TRX_DEPOSITO AS DECIMAL) * 1.#percent) - ACT_IMP_MONTO_TOT_DEPOSITOS) < 0 AND
         IMP_MONTO_ESTIMADO_TRX_DEPOSITO > 0) OR
        (((cast(NO_OPERACIONES_RETIRO AS DECIMAL) * 1.#percent) - ACT_NO_TOT_RETIROS) < 0 AND
         NO_OPERACIONES_RETIRO > 0) OR
        (((cast(IMP_MONTO_ESTIMADO_TRX_RETIRO AS DECIMAL) * 1.#percent) - ACT_IMP_MONTO_TOT_RETIROS) < 0 AND
         IMP_MONTO_ESTIMADO_TRX_RETIRO > 0) OR
        (((CAST(NO_PAGOS_CREDITO_MENSUAL AS DECIMAL) * 1.#percent) - ACT_NO_TOT_APORTACIONES) < 0 AND
         NO_PAGOS_CREDITO_MENSUAL > 0) OR
        (((cast(IMP_MONTO_PAGOS_CREDITO_MENSUAL AS DECIMAL) * 1.#percent) - ACT_IMP_MONTO_TOT_APORTACIONES) < 0 AND
         IMP_MONTO_PAGOS_CREDITO_MENSUAL > 0))
